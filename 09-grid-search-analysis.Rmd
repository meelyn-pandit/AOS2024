---
title: "Chapter 9 - Grid Search Analysis"
author: "meelyn"
date: "2025-03-03"
...
Specify the time range of node data you want to import for this analysis. This range should cover a large time window where your nodes were in a constant location.

All node health records in this time window will be used to accurately determine the position of your nodes

```{r}
# Specify start and stop times for the nodes
node_start_time <- as.POSIXct("2023-08-01 00:00:00", tz = "GMT")
node_stop_time <- as.POSIXct("2023-08-07 00:00:00", tz = "GMT")

# Specify time range of detection data you want to pull from the raw and/or blu data tables
...
# load node_health data table and subset by start and stop times
...
node_loc_plot <- plot_node_locations(node_health_df = node_health_df,
                                     node_locs = node_locs,
...
node_map <- map_node_locations(node_locs = node_locs,
...
# load raw beep data into RStudio and subset start and stop times
...
test_grid_values <- calc_grid_values(grid_df,
                                     test_rec_df, 
                                     rssi_coefs)

solution <- subset(test_grid_values, 
                   test_grid_values$value == max(test_grid_values$value))
print(solution)

# Multilateration calculation - filter dataframe for rssi values above coefficient a
reduced_rec_df <- subset.data.frame(test_rec_df, 
                                    test_rec_df$filtered_rssi >= a)

node_w_max <- reduced_rec_df[reduced_rec_df$filtered_rssi == max(reduced_rec_df$filtered_rssi),]

# run exponential decay model using a non-linear least square (nls) approach
multilat_fit <- nls(reduced_rec_df$exp_dist ~ haversine(reduced_rec_df$lat,
                                                        reduced_rec_df$lon,
                                                        ml_lat,
                                                        ml_lon),
                      reduced_rec_df,
                      start= list(ml_lat = node_w_max$lat, 
                                  ml_lon = node_w_max$lon),
