...
[Renv](https://rstudio.github.io/renv/articles/renv.html) stands for 'R Virtual Environment', which creates reproducible environments for your R projects. Many new data scientists would download all their individual packages into one location, usually the R library folder. While this global (i.e. accessible anywhere on your computer) library is convenient, certain packages or package versions can conflict with each other, causing errors and preventing you from analyzing your data. With renv, you can download all the packages for your project in your project directory and specify the package version, which will lead to fewer problems due to conflicting packages/versions.
...
# install all the packages from the 'renv.lock' file
renv::restore()
```
