---
title: "Chapter 5 - Calibrating Node Grid"
author: "meelyn"
date: "2025-02-27"
...
create_outpath('./data/meadows/sidekick/')
...
### 5.1.1 IF YOU DO NOT HAVE A SIDEKICK DATA FILE:
...
## 5.2 Load Node Health Data from Files
...
export_node_locations("examples/meadows/output/node_locations.csv", 
                      node_locs)
...
## 5.4 Load Station Detection from Files
```{r}
# connect to database
con <- DBI::dbConnect(duckdb::duckdb(), 
                      dbdir = database_file, 
...
# if you are working with blu data, load data from the blu table
detection_df <- tbl(con, 'blu') |>
  filter(time >= start_time && time <= stop_time) |>
  collect
...
## 5.5 Load Sidekick Calibration Data from File
```{r}
...
## 5.6 Calculate the RSSI vs. Distance Relationship
